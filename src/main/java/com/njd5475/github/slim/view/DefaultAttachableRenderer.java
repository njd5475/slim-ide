...
public class DefaultAttachableRenderer implements SlimRenderVisitor {
	...
	public void render(SlimRenderContext ctx, SlimFileWrapper slimFileWrapper) {
		...
		currentLineG.drawString(
				"Current File: " + slimFileWrapper.getFile().getName(), 0, 0);
		...
		cursor.setColor(new Color(Color.blue.getRed(), Color.blue.getGreen(),
				Color.blue.getBlue(), 100));
		cursor.fillRect(0, 0, cursor.getClipBounds().width,
				lineHeight + maxDescent);
		cursor.setColor(Color.black);
		cursor.drawLine(0, 0, cursor.getClipBounds().width, 0);
		cursor.dispose();
		currentLineG.drawString("Total lines " + slimFileWrapper.getLines().size(),
				0, lineHeight - maxDescent);
		...
		if (context.getNextFile() != null) {
			...
			if (context.hasFiles()) {
				for (SlimFileWrapper wrapper : context.getFiles()) {
					...
					g.translate(0,
							(wrapper.getLineCount() * lineHeight) + lineHeight + maxDescent);
				}
			} else {
	...
	public void render(SlimRenderContext ctx, SlimLineWrapper slimLineWrapper) {
		...
		if (currentLine == cursorLine) {
			...
			cursor.setColor(new Color(Color.yellow.getRed(), Color.yellow.getGreen(),
					Color.yellow.getBlue(), 100));
			cursor.fillRect(0, -lineHeight, cursor.getClipBounds().width,
					lineHeight + maxDescent);
			cursor.dispose();
		}
	...
	public void render(SlimRenderContext ctx, SlimSymbolWrapper symbol) {
		...
		for (char c : symChars) {
			if (currentChar == cursorColumn && currentLine == cursorLine) {
				...
				block.drawLine(0, -lineHeight + 1, 0,
						block.getFontMetrics().getMaxDescent());
				block.dispose();
			}
			if (c != '\t') {
